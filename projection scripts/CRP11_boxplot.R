...
# Intense precipitation indicator: CRP11
...
# DIRECTORY
# -----------------------------
input_dir <- "/lustre/nobackup/WUR/ESG/sethi002/Climate Projection Output/Affected_Maps_CRP11/"

output_dir <- "/lustre/nobackup/WUR/ESG/sethi002/Climate Projection Output/CRP11 Box Plot/"
  ...
  for (ssp in ssps) {
    ...
    if (ssp != "historical") {
      ...
      for (yr in periods$MidCentury) {
        ...
        for (cft in cfts) {
          pattern <- paste0(
            "^affected_area_crp11_event_map_",
            cft,
            "_",
            model,
            "_",
...
ggsave(
  filename = file.path(output_dir, "affected_area_boxplots_CRP11.png"),
  plot = p,
  width = 14,
  height = 10,
  dpi = 300
...
write.csv(results,
          file.path(output_dir, "affected_area_boxplot_data_CRP11.csv"),
          row.names = FALSE)
