...
base_dir  <- "/lustre/nobackup/WUR/ESG/sethi002/Climate Projection Output/Future_GS_CRP11_events"
wheat_dir <- "/lustre/nobackup/WUR/ESG/sethi002/Wheat_Maps"
out_dir   <- "/lustre/nobackup/WUR/ESG/sethi002/Climate Projection Output/Affected_Maps_CRP11"
...
for (fld in folders) {
  ...
  for (f in files_year) {
    ...
    out_binary <- file.path(
      out_subdir,
      paste0("affected_binary_crp11_", fname, ".tif")
    )
    
    out_area <- file.path(
      out_subdir,
      paste0("affected_area_crp11_", fname, ".tif")
    )
